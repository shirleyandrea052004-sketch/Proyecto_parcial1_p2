...
# Modificado para soportar 3 agentes (ego, opp, opp2)
...
class GymBridge(Node):
    def __init__(self):
        ...
        # --- NUEVOS PARÁMETROS PARA EL TERCER AUTO (opp2) ---
        self.declare_parameter('sx2', 0.0)
        self.declare_parameter('sy2', -2.0)
        self.declare_parameter('stheta2', 0.0)
        self.declare_parameter('opp2_namespace', 'opp2_racecar')
        ...
        if num_agents < 1 or num_agents > 3:
            raise ValueError('num_agents should be between 1 and 3.')
        ...
        # Setup opp2 (Auto 3) e inicializar entorno
        if num_agents == 3:
            self.has_opp2 = True
        ...
        if self.has_opp2:
            ...
            self.opp2_reset_sub = self.create_subscription(PoseStamped, '/goal_pose2', self.opp2_reset_callback, 10)
    ...
    def ego_reset_callback(self, pose_msg):
        ...
        if self.has_opp2:
            ...
            opp2_pose = [self.obs['poses_x'][2], self.obs['poses_y'][2], self.obs['poses_theta'][2]]
            self.obs, _ , self.done, _ = self.env.reset(np.array([[rx, ry, rtheta], opp_pose, opp2_pose]))
        elif self.has_opp:
            opp_pose = [self.obs['poses_x'][1], self.obs['poses_y'][1], self.obs['poses_theta'][1]]
    ...
    def opp_reset_callback(self, pose_msg):
        if self.has_opp:
            ...
            if self.has_opp2:
                opp2_pose = [self.obs['poses_x'][2], self.obs['poses_y'][2], self.obs['poses_theta'][2]]
                self.obs, _ , self.done, _ = self.env.reset(np.array([list(self.ego_pose), [rx, ry, rtheta], opp2_pose]))
            else:
                self.obs, _ , self.done, _ = self.env.reset(np.array([list(self.ego_pose), [rx, ry, rtheta]]))
            self._update_sim_state()

    def opp2_reset_callback(self, pose_msg):
        if self.has_opp2:
            rx = pose_msg.pose.position.x
            ry = pose_msg.pose.position.y
            rqx = pose_msg.pose.orientation.x
            ...
            rqw = pose_msg.pose.orientation.w
            _, _, rtheta = euler.quat2euler([rqw, rqx, rqy, rqz], axes='sxyz')
            self.obs, _ , self.done, _ = self.env.reset(np.array([list(self.ego_pose), list(self.opp_pose), [rx, ry, rtheta]]))
            self._update_sim_state()
    ...
    def drive_timer_callback(self):
        ...
        elif self.ego_drive_published and self.has_opp2 and self.opp_drive_published and self.opp2_drive_published:
    ...
    def _publish_odom(self, ts):
        ...
        if self.has_opp2:
            ...
            opp2_odom.twist.twist.linear.x = self.opp2_speed[0]
            opp2_odom.twist.twist.linear.y = self.opp2_speed[1]
            opp2_odom.twist.twist.angular.z = self.opp2_speed[2]
    ...
    def _publish_transforms(self, ts):
        ...
        if self.has_opp2:
            opp2_t = Transform()
            opp2_t.translation.x = self.opp2_pose[0]
            opp2_t.translation.y = self.opp2_pose[1]
            opp2_t.translation.z = 0.0
            opp2_quat = euler.euler2quat(0.0, 0.0, self.opp2_pose[2], axes='sxyz')
            opp2_t.rotation.x = opp2_quat[1]
            opp2_t.rotation.y = opp2_quat[2]
            opp2_t.rotation.z = opp2_quat[3]
            opp2_t.rotation.w = opp2_quat[0]

            opp2_ts = TransformStamped()
            opp2_ts.transform = opp2_t
            opp2_ts.header.stamp = ts
            opp2_ts.header.frame_id = 'map'
            opp2_ts.child_frame_id = self.opp2_namespace + '/base_link'
    ...
    def _publish_wheel_transforms(self, ts):
        ...
        if self.has_opp2:
            ...
            opp2_wheel_ts.transform.rotation.x = opp2_wheel_quat[1]
            opp2_wheel_ts.transform.rotation.y = opp2_wheel_quat[2]
            opp2_wheel_ts.transform.rotation.z = opp2_wheel_quat[3]
            opp2_wheel_ts.transform.rotation.w = opp2_wheel_quat[0]
